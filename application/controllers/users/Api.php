...
class Api extends MY_RootController {
   ...
   function users_get(){
       $sql = "SELECT id_person, concat(name_person, ' ', lastname_person) as fullname_person, email_person, gender_person, identifier_person, phone_person, role_person, IF((SELECT COUNT(*) FROM tb_users WHERE user_person = id_person)>0,'Con acceso', 'Sin acceso') AS access_person, IF(person_career IS NULL, '', (SELECT name_career FROM tb_careers WHERE person_career = id_career)) AS career_person FROM tb_persons";
   ...
   function web_users_post(){
     ...
     $this->form_validation->set_rules('pGenre','Género','required|in_list[F,M]');
     ...
     $this->form_validation->set_rules('pPhone','Télefono','required|max_length[18]');
     if($this->form_validation->run()){
         $this->DAO->init_transaction();
         $data_person = array(
             "name_person" => $this->post('pName'),
             "lastname_person" => $this->post('pLastname'),
             "gender_person" => $this->post('pGenre'),
         ...
         $this->DAO->saveOrUpdate('tb_users',$data_user);
         ...
         if($this->DAO->check_transaction()['status']=="success"){
             ...
             $response=array(
                 "status"=>"error",
                 "message"=>"Error al crear el usuario, intente nuevamente, si el error persiste contacte al admin",
                 "validations"=>array(),
                 "data"=>array()
             );
         }
   ...
   function mobile_users_post(){
     ...
     $this->form_validation->set_rules('pIdentifier','Identificación[RFC/Num. Empleado]','required|max_length[18]|min_length[3]|is_unique[tb_persons.identifier_person]');
     $this->form_validation->set_rules('pPhone','Télefono','required|max_length[18]');
     $this->form_validation->set_rules('pRole','Rol','required|in_list[Estudiante, Personal acad├®mico/administrativo]');
     if($this->form_validation->run()){
   ...
   function career_exists($value){
        $careers_exists = $this->DAO->selectEntity('tb_careers',array('id_career'=>$value,'status_career'=>'Activo'),TRUE);
